...
export function parseTiledMap(_input: unknown): ParsedMapContract {
  ...
  for (const value of spawnLayer.objects) {
    ...
    if (value.visible === false) {
      const objectLabel = value.name === "player"
        ? "player"
  ...
  for (const value of collisionLayer.objects) {
    ...
    const objectLabel = Object.hasOwn(collision, "id") ? ` ${String(collision.id)}` : "";
    ...
    if (
      ...
      throw new Error(`Collision object${objectLabel} must be a visible grid-aligned rectangle inside the map`);
    }
